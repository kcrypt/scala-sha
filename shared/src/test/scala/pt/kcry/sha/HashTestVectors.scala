...
class HashTestVectors extends wordspec.AnyWordSpec with should.Matchers {
  ...
  "DI Management test vectors" when {
    "length 24 bits" in {
      ...
      "abc" shake_256_ShouldBe
        "483366601360a8771c6863080cc4114d8db44530f8f1e1ee4f94ea37e78b57"
    }
    ...
    "length 0 bits" in {
      ...
      "" shake_256_ShouldBe
        "46b9dd2b0ba88d13233b3feb743eeb243fcd52ea62b81b82b50c27646ed576"
    }
    ...
    "length 448 bits" in {
      ...
      input shake_256_ShouldBe
        "4d8c2dd2435a0128eefbb8c36f6f87133a7911e18d979ee1ae6be5d4fd2e33"
    }
    ...
    "length 896 bits" in {
      ...
      input shake_256_ShouldBe
        "98be04516c04cc73593fef3ed0352ea9f6443942d6950e29a372a681c3deaf"
    }
    ...
    "one million repetitions of the a" in {
      ...
      input shake_256_ShouldBe
        "3578a7a4ca9137569cdf76ed617d31bb994fca9c1bbf8b184013de8234dfd1"
    }
  }

  case class RichString(str: String) {
    def sha0_shouldBe(expected: Array[Byte]): Assertion = Sha0
      .hash(str.getBytes()) shouldBe expected

    def sha1_shouldBe(expected: Array[Byte]): Assertion = Sha1
      .hash(str.getBytes()) shouldBe expected

    def sha2_224_ShouldBe(expected: Array[Byte]): Assertion = Sha2_224
      .hash(str.getBytes()) shouldBe expected

    def sha2_256_ShouldBe(expected: Array[Byte]): Assertion = Sha2_256
      .hash(str.getBytes()) shouldBe expected

    def sha2_384_ShouldBe(expected: Array[Byte]): Assertion = Sha2_384
      .hash(str.getBytes()) shouldBe expected

    def sha2_512_ShouldBe(expected: Array[Byte]): Assertion = Sha2_512
      .hash(str.getBytes()) shouldBe expected

    def sha3_224_ShouldBe(expected: Array[Byte]): Assertion = Sha3_224
      .hash(str.getBytes()) shouldBe expected

    def sha3_256_ShouldBe(expected: Array[Byte]): Assertion = Sha3_256
      .hash(str.getBytes()) shouldBe expected

    def sha3_384_ShouldBe(expected: Array[Byte]): Assertion = Sha3_384
      .hash(str.getBytes()) shouldBe expected

    def sha3_512_ShouldBe(expected: Array[Byte]): Assertion = Sha3_512
      .hash(str.getBytes()) shouldBe expected

    def shake_128_ShouldBe(expected: Array[Byte]): Assertion = Shake_128
      .hash(str.getBytes(), 13) shouldBe expected

    def shake_256_ShouldBe(expected: Array[Byte]): Assertion = Shake_256
      .hash(str.getBytes(), 31) shouldBe expected
  }
